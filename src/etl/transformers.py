...
def transform_church(raw: dict, diocese_id: int | None = None) -> dict:
    ...
    if state_name and state_name not in STATE_MAP:
        logger.warning(f"Unknown state '{state_name}' for church {raw.get('name')} — using '{state_code}'")
    ...
    if raw.get("updatedAt"):
        try:
            ...
            logger.warning(f"Could not parse updatedAt '{raw.get('updatedAt')}' for {raw.get('name')}")
