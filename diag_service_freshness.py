...
def main():
    ...
    L.append(
        f"active service rows            {one('SELECT COUNT(*) FROM service WHERE is_active=1'):,}"
    )
    L.append(
        f"churches with >=1 service      {one('SELECT COUNT(DISTINCT church_id) FROM service WHERE is_active=1'):,}"
    )
    L.append(
        f"churches with >=1 bulletin_pdf {one('SELECT COUNT(DISTINCT bs.church_id) FROM bulletin_source bs JOIN bulletin_pdf bp ON bp.bulletin_source_id=bs.bulletin_source_id'):,}"
    )
    L.append("")
    L.append("--- when were services last written? ---")
    cur.execute(
        "SELECT DATE(updated_at) d, COUNT(*) n FROM service WHERE updated_at IS NOT NULL GROUP BY DATE(updated_at) ORDER BY d DESC LIMIT 8"
    )
    ...
    cur.execute(
        "SELECT DATE(last_scraped_at) d, COUNT(*) n FROM church WHERE last_scraped_at IS NOT NULL GROUP BY DATE(last_scraped_at) ORDER BY d DESC LIMIT 6"
    )
    for r in cur.fetchall():
        L.append(f"  {r['d']}  {r['n']:,}")
    ...
    publish("diag_service_freshness", out)
...
if __name__ == "__main__":
    try:
        ...
        print(tb, flush=True)
        publish("diag_service_freshness", tb)
        sys.exit(1)
