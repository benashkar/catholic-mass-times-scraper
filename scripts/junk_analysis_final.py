...
SAFE_SURNAMES_LOWER = {s.lower() for s in {
    ...
    'Cross','Chapel','Bishop','Pope','Priest','Deacon',
    ...
    'Ash','Oak','Birch','Pine','Elm','Willow','Rowan',
    'Burns','Burn','Dale','Glen','Moor',
    'Shields','Power','Powers','Hand','Hands','Head',
    'Law','Laws','Rule','Rush',
    'Mark','Marks','Johns','Patrick',
    ...
    'Todd','Scott','Craig','Brad','Chad','Troy',
    'Luz','Del','Paz',
    'Post','Gift','Gill','Pike','Bass','Crane','Hawk',
}}
...
COMMON_NOUNS_VERBS = {
    ...
    'summer','winter','autumn','christmas','easter','lent','advent',
    ...
    'cemetery','memorial','remembrance','deceased',
    ...
    'chapter','psalm','psalms','hymnal',
    'penance','reconciliation','anointing',
}

PLACE_WORDS = {
    'park','city','county','village','township','town','heights','hills',
    'springs','creek','river','lake','valley','mountain','ridge','grove',
    'forest','woods','meadow','prairie','plains',
...
COMMON_FIRST_NAMES = {
    ...
    'bruce','alan','willie','gabriel','alexander','aaron','randy','ray','joel',
    'tommy','adam','sean','nathan','jesse','craig','billy','todd','johnny','phillip',
    'terry','tony','allen','douglas','leonard','harold','stanley','dylan','logan',
    'luke','alex','connor','jose','mike','jim','bob','bill','tom','dan','ed','ted','tim',
}

TRUNC_ENDINGS = [
    'comuni','collectio','photog','registr','celebr',
    'informat','organiz','coordin','particip','volunt',
    'evangel','catec','reconcili',
...
print(f"\n" + "="*80)
print(f"  JUNK DETECTION: {len(all_junk):,} of {len(unique_names):,} unique names flagged ({100.0*len(all_junk)/len(unique_names):.1f}%)")
print(f"  Clean names remaining: {len(all_clean):,} ({100.0*len(all_clean)/len(unique_names):.1f}%)")
print(f"="*80)
...
for conf_level in ['high', 'medium']:
    ...
    print(f"    {conf_level:10s}: {total:>8,} total, {junk:>8,} junk ({rate:5.1f}%), {clean:>8,} clean")
...
for st in sorted(state_counts.keys()):
    ...
    print(f"    {st:15s} {state_counts[st]:>8,} {len(st_names):>8,} {st_junk:>7,} {rate:>6.1f}% {st_clean:>8,}")
...
for n in newline_names:
    ...
    if parts:
        ...
        if 2 <= len(words) <= 3 and all(w[0].isupper() for w in words if w):
            first_line_clean += 1
print(f"    First line looks like valid 2-3 word name: {first_line_clean:,} ({100.0*first_line_clean/len(newline_names):.1f}%)")
